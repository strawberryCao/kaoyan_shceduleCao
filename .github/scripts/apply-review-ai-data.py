...
# Organizer: evidence-first wrong reasons, user ownership, and targeted single-note runs.
...
# Learning store: preserve manual edits/deletions and expose provenance.
...
# Note server: targeted background enrichment, manual sidecar ownership, and nonblocking routes.
...
# Add targeted queue before naming queue.
text = replace_once(
    text,
...
# Dedicated re-analysis endpoint must be matched before generic note route handling.
text = replace_once(
    text,
...
print('Applied wrong-reason persistence and targeted enrichment changes.')
