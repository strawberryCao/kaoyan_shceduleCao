...
type CanvasRequestOperation = 'list' | 'load' | 'save' | 'delete';
...
export type CanvasProjectEvent = CanvasProjectSavedEvent | CanvasProjectLiveStrokeEvent | CanvasProjectDeletedEvent;
...
export function subscribeCanvasProjectEvents(
  ...
  const handleEvent = (message: Event) => {
    try {
      ...
      if (event.type !== 'saved' && event.type !== 'live-stroke' && event.type !== 'deleted') return;
      onEvent(event);
    } catch {
