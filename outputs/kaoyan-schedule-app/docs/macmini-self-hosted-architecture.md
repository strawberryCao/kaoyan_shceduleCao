...
4. 移动端不保存完整业务库；仅允许保存“尚未收到 Mac 确认”的 AES-GCM 加密临时发件箱，确认后立即删除。
...
1. 使用浏览器 WebCrypto 生成的不可导出 AES-GCM 密钥，在写入 IndexedDB 前加密正文、元数据和附件。
2. 本地只保存不可直接读取的密文、不可导出密钥、操作 ID、重试信息和最小展示状态；这一层保护磁盘静态内容，但不把受恶意同源脚本控制的浏览器描述为端到端可信环境。
...
- 逐项询问实际启用的 AI 提供商与 API Key；Qwen、Kimi、DeepSeek 使用经过项目验证的内置服务地址和模型，Gemini 仅额外询问服务提供地址；密钥输入不回显。
...
第二阶段有意继续使用现有文件/JSON 服务逻辑，以便先完成跨平台运行边界。详细命令、目录和实机验收门槛见 `docs/macmini-phase-2-runbook.md`。

### 11.2 第三阶段实现映射

第三阶段在既有 JSON 应用层之外增加了独立同步层：Mac 使用 SQLite 保存实体、字段时钟、幂等操作回执、事件游标、冲突和附件索引；Windows 使用另一份 SQLite 保存完整投影、离线发件箱、设备序列与下行游标。学习笔记、卡片、每日人工记录和画布会在本地持久化之后进入同步层，附件转换为 SHA-256 内容寻址对象。Windows 新运行时不会读取本机旧 AI 密钥，需 AI 的操作只显示等待 Mac。

Mac 网关只在显式启用的 loopback ingress 模式下接受 Tailscale/Cloudflare 转发的主机名；`/sync/v1/*` 保留设备 Bearer 令牌并拒绝带浏览器 Origin 的请求，普通 `/api/*` 仍剥离认证信息并受原路由白名单约束。设备配置只保存令牌哈希，令牌只在创建时显示一次。

这一层已经通过临时数据验证，但还没有把现有 Windows、GitHub 或 Cloudflare 正式数据导入 Mac。因此现在仍不得把 Mac 声明为正式权威源。具体边界和演练方式见 `docs/macmini-phase-3-runbook.md`。

### 11.3 第四阶段实现映射

第四阶段新增 Mac SQLite 持久化 AI 队列：业务内容先落盘，provider 请求前记录尝试，已发出但未确认的请求在重启后进入人工确认，不会自动重复付费。任务状态通过同步层回到 Windows，活动中心展示请求次数和显式重试。

远程 HTTPS 页面使用 Mac 本地配置的口令派生验证与签名会话；Tailscale Serve 配置脚本默认只读，应用前要求登录配置和 loopback readiness，并永不启用 Funnel。手机/iPad 的正式业务快照只在页面内存，未确认捕获以 AES-GCM 密文暂存，Safari `online`、`pageshow` 和前台恢复会续传可安全重放的上传，Mac 确认后清除密文。
