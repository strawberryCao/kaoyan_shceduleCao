...
状态：设计合同已冻结；第三阶段同步协议与 Windows 本地副本基础层已实现，尚未切换正式数据
...
这一层已经通过临时数据验证，但还没有把现有 Windows、GitHub 或 Cloudflare 正式数据导入 Mac，也没有建立移动端加密发件箱、持久化 AI 任务队列或正式入口。因此现在仍不得把 Mac 声明为正式权威源。具体边界和演练方式见 `docs/macmini-phase-3-runbook.md`。
