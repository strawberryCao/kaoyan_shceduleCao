...
function providerSecretRef(providerId) {
  ...
  return SECRET_REFS[id] || `${String(providerId || 'PROVIDER').toUpperCase().replace(/[^A-Z0-9]+/g, '_')}_API_KEY`;
}
...
function sanitizeValue(value, report, context = {}) {
  ...
  for (const [key, item] of Object.entries(value)) {
    if (SENSITIVE_NAME.test(key)) {
      ...
      const reference = context.providerId ? providerSecretRef(context.providerId) : `LOCAL_${key.toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`;
      result[key] = `__SECRET_REF:${reference}__`;
    } else {
      result[key] = sanitizeValue(item, report, context);
    }
  }
  return result;
}
...
function sanitizeProviders(value, report) {
  ...
  const entries = Array.isArray(value) ? value.map((provider) => [provider?.id, provider]) : Object.entries(value);
  const result = {};
...
function buildTaskContracts(configuredTasks) {
  ...
  return Object.fromEntries(Object.entries(AI_TASK_DEFINITIONS).map(([taskId, definition]) => {
    ...
    return [taskId, {
      ...
      profile: { difficulty: profile.difficulty, capabilities: [...profile.capabilities] },
...
function main() {
  ...
  for (const filePath of allJson) {
    ...
    if (!includedClasses.has(category)) {
    ...
    const targetPath = path.join(safeFilesRoot, relativePath);
    writeJson(targetPath, sanitized);
    const bytes = fs.readFileSync(targetPath);
    includedFiles.push({ path: relativePath, category, bytes: bytes.length, sha256: sha256(bytes) });
  }
  includedFiles.sort((left, right) => left.path.localeCompare(right.path, 'zh-CN'));
  ...
  const providers = sanitizeProviders(aiConfig.providers || {}, report);
  if (!providers.qwen) {
    const qwenBaseUrl = sanitizeString(String(legacyQwen.baseUrl || ''), report);
    providers.qwen = {
      id: 'qwen',
      enabled: true,
      ...
      cloudUsable: Boolean(qwenBaseUrl) && qwenBaseUrl !== '__LOCAL_ONLY_URL__',
      models: [{ id: String(legacyQwen.model || 'qwen-vl-plus') }],
    };
  }
  for (const [providerId, catalog] of Object.entries(PROVIDER_MODEL_CATALOG)) {
    if (providers[providerId]) providers[providerId].catalog = [...catalog];
  }
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    includedFiles: includedFiles.length,
    excludedFiles: allJson.length - includedFiles.length,
    excludedCounts,
  })}\n`);
}
