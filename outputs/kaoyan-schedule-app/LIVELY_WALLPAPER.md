# 考研桌面助手使用说明

现在推荐只记一个入口：

```text
启动考研桌面助手.cmd
```

它会同时启动：

```text
http://127.0.0.1:5173/?hub=1       统一入口
http://127.0.0.1:5173/?wallpaper=1 壁纸页，给 Lively 使用
http://127.0.0.1:5173/?console=1   桌面控制台
http://127.0.0.1:5173/?notes=1&mode=canvas 关联画布
http://127.0.0.1:5174/health       笔记保存服务状态
```

旧的 `启动壁纸模式.cmd`、`启动笔记台.cmd`、`启动桌面控制台.cmd` 先保留为备用入口，但日常只用 `启动考研桌面助手.cmd`。

## 先安装依赖

第一次运行先双击：

```text
安装依赖.cmd
```

它会检查 Node.js / npm，安装项目依赖，并执行构建验证。

## Lively Wallpaper 使用

Lively Wallpaper 里添加网页 URL：

```text
http://127.0.0.1:5173/?wallpaper=1
```

桌面布局不要直接在 Lively 里改，打开控制台：

```text
http://127.0.0.1:5173/?console=1
```

控制台里拖动、缩放、添加、隐藏组件后会实时同步到壁纸页，不需要手动刷新。

壁纸页右上角还固定保留了“控制台 / 添加模块”入口。这个入口不属于桌面模块，无法被布局删除；即使快捷入口模块被隐藏，也能随时重新进入控制台。

## 壁纸资源与动态引擎

清晰原图始终作为最底层画质源：

```text
public/dunhuang-wallpaper.png
```

动态视频按以下顺序自动寻找：

```text
public/dunhuang-master.webm
public/dunhuang-master.mp4
public/dunhuang-reference.mp4
```

当前采用“清晰原图 + 半透明动态视频”的混合渲染：

- 原图始终可见，用于保留山体、建筑和纹理细节。
- 视频只以约 52% 的最大透明度叠加，主要贡献丝绸与沙尘运动，避免低细节视频独占整个画面。
- 视频在开始后约 1.05 秒内平滑出现，在结束前约 1.05 秒内平滑隐去。
- 视频回到首帧时已经完全透明，切帧发生在不可见状态，不再直接把尾帧叠到首帧上。
- 已彻底删除 WebGL 图片扭曲、椭圆径向光晕、`screen` 高光和鼠标驱动的整图形变。

## 鼠标交互

鼠标交互只发生在独立 Canvas 沙尘层，不改变视频或原图像素：

- 常驻约 78 个小圆点浮尘。
- 鼠标附近约 168px 范围内会形成推开与轻微旋转绕流。
- 鼠标移动时生成少量点状尘埃尾迹，最大粒子总数限制为 150。
- 全部粒子都是圆点，不绘制速度线、长条、放射光束或径向光晕。
- 鼠标经过桌面组件、控制台和固定入口时不会喷发粒子。

## 导入已有动态视频

把 MP4 或 WebM 直接拖到：

```text
导入动态壁纸视频.cmd
```

脚本会复制为正式动态资产。已有的：

```text
ce23349f07467f40911d33ca2a59107c_raw.mp4
```

也可以由脚本从下载目录或桌面自动查找。

## 笔记保存

Lively 壁纸和普通 Web 页面里的文件拖拽权限不稳定，因此单图保存只放在 Electron 笔记小 App 中。可从 Web 的“快速记图”、桌面控制台或下面的备用脚本打开：

```text
启动笔记台.cmd
```

Electron 笔记小 App 支持单图拖拽、Ctrl+V、选择文件、写备注和保存。Web 不再提供重复的单图保存视图。

需要整理多张图及其关系时，Web 只保留关联画布：

```text
http://127.0.0.1:5173/?notes=1&mode=canvas
```

画布支持：

- 大画布拼图。
- 图片拖动、缩放、删除。
- 文字添加、移动、删除。
- 画笔书写，线条可选中删除。
- 保存时调用千问结合图片和备注命名。

保存位置默认是：

```text
C:\Users\ASUS\Desktop\笔记
```

## 配置千问智能命名

双击：

```text
配置千问命名.cmd
```

输入 DashScope / 千问 API Key。脚本会把 Key 写到当前 Windows 用户环境变量，不会写进 GitHub：

```text
QWEN_API_KEY
QWEN_MODEL
QWEN_BASE_URL
```

配置完成后，关闭旧服务，重新双击：

```text
启动考研桌面助手.cmd
```

打开下面地址检查：

```text
http://127.0.0.1:5174/health
```

看到：

```json
"enabled": true
```

说明千问命名已启用。

文件名会类似：

```text
高数_极限夹逼准则例题_20260708_121530.png
数据结构_二叉树遍历错题_20260708_121700.png
```

如果没有配置 Key，或者千问调用失败，会用备注、类型和时间兜底命名，不再用随机串当主文件名。

## 为什么不用 Electron 贴桌面

之前的 Electron 方案会尝试使用 Windows 的 `WorkerW / Progman / SetParent` 把窗口强行挂到桌面层。这个方案容易出现窗口坐标错乱、固定位置跳动、层级异常等问题。

现在把真正的壁纸层交给 Lively Wallpaper。Lively Wallpaper 免费，适合把本地网页作为桌面壁纸加载。

## 开机自动使用

双击：

```text
设置开机自启.cmd
```

它会在 Windows 启动文件夹创建快捷方式，用于开机后隐藏启动本地服务。

同时在 Lively Wallpaper 设置里打开：

```text
Start with Windows / 随 Windows 启动
```

如果以后不想开机自动启动本地服务，双击：

```text
取消开机自启.cmd
```
