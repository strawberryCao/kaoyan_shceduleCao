...
仓库已经内置以下资源，不再依赖 Downloads 目录临时导入：

```text
public/dunhuang-wallpaper.png  1920x1080 静态底图/视频封面
public/dunhuang-loop.mp4       1920x1080、24fps、10 秒无缝循环视频
```

视频锁定镜头，只让丝绸区域做小幅周期运动；沙尘由网页粒子层渲染。页面只播放一个视频层，避免重复视频叠加形成椭圆光晕。
